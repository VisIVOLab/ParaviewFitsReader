...
int vtkFitsReader::RequestData(vtkInformation *, vtkInformationVector **, vtkInformationVector *outVec)
{
    ...
    int anynul = 0;
    bool ok = true;
    vtkSMPTools::For(1, nels, sqrtl(nels), [&](long start, long end) {
        long long n = end - start + 1;
        float *offset = ptr + start - 1;

        std::lock_guard<std::mutex> lk(m);
        if (ok && fits_read_img(fptr, TFLOAT, start, n, &nulval, offset, &anynul, &ReadStatus))
        {
            vtkErrorMacro(<< "(#" << ProcId << ") [CFITSIO] Error fits_read_img");
            fits_report_error(stderr, ReadStatus);
            ok = false;
        }
    });
    ...
    if (!ok)
    {
        vtkErrorMacro(<< "(#" << ProcId << ") Error during fits_read_img in vtkSMPTools::For");
        return 0;
    }
